<?php include("../inc/inc_header.php") ?>
		...
		<?php 
			...
			if($op == 'delete'){
				$id = $_GET['id'];
				$query	= "SELECT struktur_img FROM pemdas WHERE id = '$id'";
				$result	= mysqli_query(connection(), $query);
				$data	= mysqli_fetch_array($result);
				@unlink("../../images/".$data['stuktur_img']);

				$query 	= "DELETE FROM pemdas WHERE id = '$id'";
				$result = mysqli_query(connection(), $query);
				...
				if ($result) {
		            echo "
		            		<script>
		            			...
		            			document.location.href = 'pemdas.php';
							</script>
						";
		        } else{
		            echo "
		...
		<table class="table table-striped">
			...
			<tbody>
				...
				<?php
					...
					// QUERY DATA KE TABEL
					$query = "SELECT * FROM pemdas $querytambahan ORDER BY id DESC";
					$result = mysqli_query(connection(), $query);
					$number = 1;
					...
					?>
						<tr>
							...
							<td>
								<a href="pemdas.php?op=delete&id=<?php echo $data['id']; ?>" onclick="return confirm('Apakah anda yakin ?')"> 
									<span class="badge bg-danger">Hapus</span>
								</a>
								<a href="pemdas_input.php?id=<?php echo $data['id']; ?>">
